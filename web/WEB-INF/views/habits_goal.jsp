...
<head>
    ...
    <%
        ...
        if (habitList == null) {
            habitList = java.util.Collections.emptyList();
        }

        Goal goal = (Goal) request.getAttribute("goal");

        // Safely handle a possibly null Goal: expose safe strings for id and name
        String goalIdStr = "";
        String goalNameStr = "";
        if (goal != null) {
            // goal.getId() is a primitive (int) in the model; just convert it to string
            goalIdStr = String.valueOf(goal.getId());
            goalNameStr = (goal.getName() != null) ? goal.getName() : "Unnamed Goal";
        } else {
            goalNameStr = "All Goals"; // fallback when no specific goal is provided
        }
    ...
    <div class="header">
        <div class="header-top">
            <div>
                ...
                <h1 class="header-title">Goal Overview</h1>
        ...
        <div class="goal-header">
                    <%-- Previous/Next only shown when a specific goal is available --%>
                    <% if (!goalIdStr.isEmpty()) { %>
                        <form method="get" action="<%=request.getContextPath()%>/goals/<%=goalIdStr%>/previous" style="display:inline;">
                            <button type="submit" class="nav-prev-btn">&#8592;</button>
                        </form>
                    <% } %>

                    <span class="goal-name"><%= goalNameStr %></span>
                    <% if (!goalIdStr.isEmpty()) { %>
                        <span><%=goalIdStr%></span>
                    <% } %>

                    <% if (!goalIdStr.isEmpty()) { %>
                        <form method="get" action="<%=request.getContextPath()%>/goals/<%=goalIdStr%>/next" style="display:inline;">
                            <button type="submit" class="nav-next-btn">&#8594;</button>
                        </form>
                    <% } %>
        </div>
